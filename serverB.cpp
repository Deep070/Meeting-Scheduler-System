...
//used to send usernames to user
...
//used to send intersection of time availability
int sendUDPData2(int sockfd, const std::string availabilities, const char* hostname, const char* port) 
{
    ...
    std::cout << "Server B finished sending the response to Main Server \n"; 
    return numBytesSent;
}
...
//parsing string to TimeAvailability
...
int main()
{
    ...
    std::cout << "Serverb is up and running using UDP on port " << PORT << "." << std::endl;
    string fileB = "b.txt";
    vector<User> usersB = readInputFile(fileB);
    const void* data = &usersB[0];
        size_t dataSize = usersB.size() * sizeof(User);
     ...
     while (true) 
    {
        ...
        std::string username;
        while (iss >> username) {
            usernamesB.push_back(username);
        }

        if (!usernamesB.empty())
         {
            
            std::cout << "Server B received the usernames from Main Server using UDP over port "<< PORT << std::endl;

            vector<User> usersWithMatchingNames;

            for (size_t i = 0; i < usernamesB.size(); ++i) 
            {
                for (size_t j = 0; j < usersB.size(); ++j) 
                {
                    if (usersB[j].username == usernamesB[i]) 
                    {
                        usersWithMatchingNames.push_back(usersB[j]);
                    }
                }
            }
            ...
            else
            {
              ...
              std::cout << "Found the intersection result: ";
                ...
                for (int i = 0; i < usernamesB.size(); i++)
                { 
                    if (i != usernamesB.size() - 1)
                    {
                        std::cout << usernamesB[i] << ",";
                    }
                    else
                    {
                    std::cout << usernamesB[i]<< ".";
            ...
            for (size_t i = 0; i < usersWithMatchingNames.size(); ++i) 
            {
                for (size_t j = 0; j < usersB.size(); ++j) 
                {
                    if (usersB[j].username == usersWithMatchingNames[i].username) 
                    {
                        // Store the old timeAvailability for printing later
                        std::vector<TimeAvailability> oldAvailability = usersB[j].timeAvailability;
                        // Update the timeAvailability with the new values
                        usersB[j].timeAvailability.assign(usersWithMatchingNames[i].timeAvailability.begin(),usersWithMatchingNames[i].timeAvailability.end());

                        // Print the update
                        std::cout << usersB[j].username << ": updated from ";
                        printAvailability(oldAvailability);
                        std::cout << " to ";
                         printAvailability(usersB[j].timeAvailability);
                         std::cout << std::endl;
                     }
                 }
            }
